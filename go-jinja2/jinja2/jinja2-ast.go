...
func ProcessJ2Test(val VariableType, test *J2Test, c *Context) (VariableType, error) {
  ...
  if test_func, ok := c.Tests[test_name]; !ok {
    return VariableType{PY_TYPE_UNKNOWN, nil}, errors.New("the test '" + test_name + "' was not found.")
  } else {
      ...
      if rhs_res, err := rhs.Eval(c); err != nil {
        ...
        if cur_res.Type == PY_TYPE_FLOAT || rhs_res.Type == PY_TYPE_FLOAT {
          ...
          if l_err != nil || r_err != nil {
            return VariableType{PY_TYPE_UNKNOWN, nil}, errors.New("unsupported op '"+ (*rhs.Op) +"' for a (fixme) and (fixme))")
          }
          switch *rhs.Op {
          case "+":
          ...
          // string join
          if *rhs.Op == "-" {
            return VariableType{PY_TYPE_UNKNOWN, nil}, errors.New("unsupported op '"+ (*rhs.Op) +"' for a (string) and (string))")
          }
...
func (self *ModFactor) Eval(c *Context) (VariableType, error) {
  ...
  case PY_TYPE_INT, PY_TYPE_BOOL:
    i_val, err := res.AsInt()
    if err != nil {
      return VariableType{PY_TYPE_UNKNOWN, nil}, errors.New("error while converting variable to an integer")
    } else {
      switch *self.Mod {
      case "+":
  ...
  case PY_TYPE_FLOAT:
    ...
    if err != nil {
      ...
      case "-":
        f_val = -f_val
      case "~":
        return VariableType{PY_TYPE_UNKNOWN, nil}, errors.New("unsupported unary operation '~' on a float")
      }
      return VariableType{PY_TYPE_FLOAT, f_val}, nil
    }
  default:
    return VariableType{PY_TYPE_UNKNOWN, nil}, errors.New("unsupported unary operation '"+(*self.Mod)+"' on a FIXME")
  }
}
//-------------------------------------------------------------------------------------------------
type Power struct {
